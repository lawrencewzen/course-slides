...
THRESHOLDS = {
    ...
    'points_single': ('.points .items (not two-col / compact)', '.item', 5,
                      'C 拆成 3+3 / 3+4，或 A 开 data-window="3"'),
    'points_two':   ('.points .items.two-col',                   '.item', 6,
                      'C 拆成 4+4 两页，或 A 开 data-window="4"'),
    'points_cmp':   ('.points .items.compact',                   '.item', 6,
                      '已 compact 仍超 6 条 → C 拆页'),
    'demo_steps':   ('.demo-steps',                              '.demo-step', 5,
                      'C 拆成 3+3，或 A 开 data-window="3"'),
    'table_rows':   ('.table-page tbody',                        'tr (exc. .total)', 6,
                      'C 先删列再拆行，或 A 开 data-window="4"（tbody）'),
    'body_paras':   ('.body',                                    '> p', 6,
                      'C 拆成 X（1/2）/ X（2/2）'),
    'flow_nodes':   ('.flow-row',                                '.flow-node', 6,
...
class SlideScanner(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'div' and 'items' in classes:
            variant = 'two' if 'two-col' in classes else ('compact' if 'compact' in classes else 'single')
            has_window = 'windowed' in classes
            entry = {'kind': 'points_items', 'variant': variant, 'count': 0, 'has_window': has_window}
...
def main():
    if len(sys.argv) != 2:
        print('usage: lint.py <deck.html>', file=sys.stderr)
        sys.exit(2)
